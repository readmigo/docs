...
│                         iOS 客户端                            │
├────────────────┬─────────────────────┬───────────────────────┤
│  AuthManager   │ SubscriptionManager │   FeatureGateService  │
│   (账号管理)    │    (订阅管理)        │     (权限网关)        │
└───────┬────────┴──────────┬──────────┴───────────┬───────────┘
        │                   │                      │
        │                   │                      ▼
...
│                        Backend API                             │
├──────────────────┬────────────────────┬───────────────────────┤
│   Auth Module    │   Users Module     │  Subscriptions Module │
│   (认证模块)      │   (用户模块)        │    (订阅模块)          │
└────────┬─────────┴─────────┬──────────┴───────────┬───────────┘
         │                   │                      │
         ▼                   ▼                      ▼
...
│                        PostgreSQL                              │
├────────────────┬──────────────────────┬───────────────────────┤
│     User       │     Subscription     │   Usage Statistics    │
└────────────────┴──────────────────────┴───────────────────────┘
```
...
1. 用户点击 "Sign in with Apple"
        ↓
2. AuthManager.signInWithApple()
        ↓
3. POST /auth/apple (Backend)
        ↓
4. 查找或创建 User 记录
        ↓
5. 返回 User + Token
        ↓
6. SubscriptionManager.refreshSubscriptionStatus()
        ↓
7. GET /subscriptions/status (Backend)
        ↓
8. 查询 Subscription 表
        ↓
9. 返回订阅状态 (tier, status, limits)
        ↓
10. FeatureGateService 更新权限缓存
```
...
1. 用户点击 "AI 解释" 按钮
        ↓
2. FeatureGateService.canUseAI()
        ↓
3. 检查 SubscriptionManager.currentTier
        ├── Pro/Premium → 返回 .allowed
        └── Free → 继续检查
                ↓
4. 检查 UsageTracker.aiCallsToday
        ├── < 5 次 → 返回 .allowedWithLimit(remaining)
        └── >= 5 次 → 返回 .restricted(reason: .dailyLimitReached)
                ↓
5. UI 根据结果显示功能或升级提示
```
...
1. 用户选择订阅套餐
        ↓
2. SubscriptionManager.purchase(product)
        ↓
3. StoreKit 2 发起购买
        ↓
4. Apple 返回交易凭证
        ↓
5. POST /subscriptions/verify (Backend)
        ↓
6. 验证 Apple 服务器
        ↓
7. 更新/创建 Subscription 记录
        ↓
8. 返回新的订阅状态
        ↓
9. SubscriptionManager.subscriptionState 更新
        ↓
10. FeatureGateService 重新计算权限
```
...
┌─────────────────────────────────────────────────────────┐
│                    AuthManager                          │
│  - 提供: currentUser, isAuthenticated                   │
│  - 被依赖: SubscriptionManager, FeatureGateService      │
└────────────────────────┬────────────────────────────────┘
                         │ 依赖
                         ▼
┌─────────────────────────────────────────────────────────┐
│                SubscriptionManager                       │
│  - 提供: currentTier, subscriptionState                 │
│  - 依赖: AuthManager (userId)                           │
│  - 被依赖: FeatureGateService                           │
└────────────────────────┬────────────────────────────────┘
                         │ 依赖
                         ▼
┌─────────────────────────────────────────────────────────┐
│                 FeatureGateService                       │
│  - 提供: canUseAI(), canSaveVocabulary(), etc.          │
│  - 依赖: SubscriptionManager, UsageTracker              │
│  - 被依赖: 所有功能模块                                   │
└────────────────────────┬────────────────────────────────┘
                         │ 依赖
                         ▼
┌─────────────────────────────────────────────────────────┐
│                    UsageTracker                          │
│  - 提供: aiCallsToday, vocabularyCount, etc.            │
│  - 依赖: AuthManager (userId)                           │
└─────────────────────────────────────────────────────────┘
```

...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  Chapter HTML                Phone Preview                API       │
│  (Database)                  (iframe)                     Response  │
│                                                                     │
│  /images/epub/   ──────►    /api/v1/content-studio/   ──────►  R2   │
│  images/cover.jpg           books/{id}/images/                      │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  1. Parse EPUB         2. Extract Chapters    3. Check Source   │
│     (epub2 lib)           (TOC/Content)          (Query DB)     │
│                                                                 │
│         │                      │                     │          │
│         ▼                      ▼                     ▼          │
│                                                                 │
│  4. If STANDARD_EBOOKS:                                         │
│     - Shift chapter orders (+1)                                 │
│     - Prepend Cover chapter (order=1)                           │
│                                                                 │
│  5. Create chapters in DB                                       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
├────────────────────────────────────────────────────────────────────┤
│                                                                    │
│  1. 检测到 SECTION 是"高元素"（h > 80% pageHeight）               │
│                                                                    │
│  2. 使用 "split by children" 策略                                  │
│     └─ 把每个 <p> 子元素作为原子单位                              │
│                                                                    │
│  3. 问题出现：                                                     │
│     ┌─────────────────────────────────────────────────────────┐   │
...
│  Phase 1: 内容扁平化                                               │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │ 输入: <section><p>A</p><p>B</p><p>C</p></section>           │  │
│  │ 输出: [<p>A</p>, <p>B</p>, <p>C</p>]                        │  │
│  │ 目的: 忽略容器元素，提取所有可见内容块                       │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                                                    │
│  Phase 2: 逐元素处理                                               │
...
│  │ 按单词边界分割长段落：                                       │  │
│  │                                                             │  │
│  │ 1. 将 innerHTML 按空格/标签拆分为 tokens                    │  │
│  │ 2. 逐 token 测量累计高度                                    │  │
│  │ 3. 超出可用高度时，在单词边界处分割                         │  │
│  │ 4. 确保分割点不在单词中间                                   │  │
│  │ 5. 孤行保护：避免页首/页尾只有1-2行                         │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

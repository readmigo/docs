...
```mermaid
graph TD
    A["Design Tokens<br>(颜色 | 字体 | 间距 | 圆角 | 阴影 | 动画)"] --> B["Theme Layer<br>(MaterialTheme | ColorScheme | Typography | Shapes)"]
    B --> C["Component Layer<br>(Atoms | Molecules | Organisms | Templates)"]
    C --> D["Screen Layer<br>(阅读器 | 书架 | 学习 | 设置 | 探索)"]
```

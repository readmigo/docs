...
graph TD
    subgraph Collect["数据采集层"]
        Crashes["崩溃<br>Crashes"]
        Metrics["性能<br>Metrics"]
        Logs["日志<br>Logs"]
        Events["用户<br>Events"]
    end
    subgraph Process["处理/上报层"]
        Crashlytics["Crashlytics"]
        Firebase["Firebase<br>Performance"]
        Sentry["Sentry"]
    end
    subgraph Visual["可视化/告警层"]
        Dashboard["Dashboard | Alerts | Reports | Analytics"]
    end
    Crashes --> Crashlytics
    Metrics --> Firebase
    Logs --> Sentry

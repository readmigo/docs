...
```mermaid
flowchart TD
    A["人工校正<br>运营人员执行校正操作"] --> B["记录日志<br>操作类型 / 选择器 / 前后内容"]
    B --> C["模式识别<br>相同选择器 / 操作类型 / 内容模式"]
    C --> D["聚类分析<br>计算哈希值 / 统计出现次数 / 涉及书籍数"]
    D --> E["置信度计算<br>出现频率 / 跨书籍一致性 / 人工确认比例"]
    E --> F["高置信度 (>=95%)"]
    E --> G["中/低置信度 (<95%)"]
    F -->|"自动转化"| H["生成规则<br>自动启用"]
    G -->|"待审批"| I["学习报告<br>人工审批"]
```
...
```mermaid
flowchart TD
    A["待审批 (pending)<br>新学习到的模式"] --> B["已批准 (approved)"]
    A --> C["已拒绝 (rejected)"]
    A --> D["自动转化 (置信度>=95%)"]
    B --> E["活跃规则 (active)<br>应用于新书籍"]
    D --> E
    E --> F["禁用 (disabled)"]
    E --> G["废弃 (deprecated)"]
```
...
> 每次校正后: 确认 -> 增加规则置信度 | 回滚 -> 降低规则置信度 | 修改 -> 记录变体，可能产生新模式

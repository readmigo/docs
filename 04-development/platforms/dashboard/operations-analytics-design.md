...
graph TD
    ...
    subgraph Collection["事件采集层"]
        Tracking["POST /tracking/reading<br>POST /tracking/ai<br>POST /tracking/batch<br>POST /tracking/app-launch"]
    end
    Clients --> Collection
    Collection --> RedisQ["Redis Queue<br>(异步处理)"]
    Collection --> PG1["PostgreSQL<br>(实时写入)"]
    subgraph Aggregation["数据聚合层"]
        Realtime["实时聚合 (Redis)<br>DAU计数 | 活跃用户"]
        Scheduled["定时聚合 (Cron Job)<br>日报表 | 月度汇总"]
        OnDemand["按需聚合 (API触发)<br>自定义报表 | 导出数据"]
    end
    RedisQ --> Aggregation
    PG1 --> Aggregation
    subgraph Storage["数据存储层"]
        PG2["PostgreSQL<br>OperationsDailyStats<br>OperationsMonthlyStats<br>HotContent"]
        Redis2["Redis<br>BitMap | HyperLogLog | Sorted Set"]
    end
    Aggregation --> Storage
    Storage --> Display["展示层 (Dashboard)<br>实时看板 | 趋势图表 | 排行榜 | 报表导出"]
```
...
```mermaid
graph TD
    A["触发<br>Cron: 每日 00:30 UTC"] --> B["获取日期<br>计算昨日日期 (UTC)"]
    B --> C["并行聚合各维度数据<br>用户 | 阅读 | 词汇 | AI<br>社交 | 订阅 | 热门内容"]
    C --> D["按平台拆分<br>ALL / iOS / Android / Web / Expo"]
    D --> E["写入数据库<br>UPSERT 到 OperationsDailyStats"]
    E --> F["更新缓存<br>刷新 Redis 缓存"]
    F --> G["发送通知<br>如有异常指标，发送告警"]
```
...
```mermaid
graph TD
    subgraph Dashboard["统一运营 Dashboard (运营人员主入口)"]
        Ops["运营数据<br>DAU/MAU | 用户画像<br>热门内容 | 留存分析"]
        Channel["渠道汇总<br>iOS下载量<br>Android下载 | 评分趋势"]
        Revenue["收入汇总<br>iOS收入 | Android收入<br>订阅转化"]
        AlertCenter["告警中心<br>崩溃告警 | API告警<br>服务器告警"]
    end
    subgraph Tools["专业工具 (技术人员入口)"]
        Sentry["Sentry<br>崩溃详情 | 堆栈追踪 | Issue管理"]
        ASC["App Store Connect<br>审核管理 | TestFlight"]
        GPC["Google Play Console<br>发布管理 | 设备兼容性"]
    end
    subgraph SelfBuilt["自建数据系统"]
        PG["PostgreSQL<br>用户行为 | 阅读/学习 | 业务指标"]
        Redis["Redis<br>实时指标 | 缓存"]
    end
    Tools -->|"数据聚合<br>(API同步/定时拉取)"| Dashboard
    SelfBuilt -->|"数据聚合<br>(API同步/定时拉取)"| Dashboard
```

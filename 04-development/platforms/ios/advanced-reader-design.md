...
graph TD
    subgraph Presentation["表现层 (Presentation Layer)"]
        ReaderView["ReaderView<br>阅读主界面"]
        AnnotationUI["AnnotationUI<br>划线标注界面"]
        SceneView3D["3DSceneView<br>3D可视化"]
        AudioPlayerUI["AudioPlayerUI<br>朗读控制"]
    end

    subgraph Business["业务逻辑层 (Business Layer)"]
        ReadingEngine["ReadingEngine<br>阅读引擎"]
        AnnotationMgr["AnnotationMgr<br>批注管理器"]
        AssetEngine3D["3DAssetEngine<br>3D资源引擎"]
        AudioEngine["AudioEngine<br>音频引擎"]
        SmartAnnotate["SmartAnnotate<br>智能注释引擎"]
        SyncManager["SyncManager<br>同步管理器"]
        AnalyticsHub["AnalyticsHub<br>数据分析中心"]
    end

    subgraph Service["服务层 (Service Layer)"]
        AIService["AIService<br>AI服务"]
        TTSService["TTSService<br>TTS服务"]
        GenService3D["3DGenService<br>3D生成服务"]
        CloudSync["CloudSync<br>云端同步"]
    end

    subgraph Data["数据层 (Data Layer)"]
        LocalStorage["LocalStorage<br>本地存储"]
        ContentCache["ContentCache<br>内容缓存"]
        AssetCache3D["3DAssetCache<br>3D资源缓存"]
        CloudDB["CloudDB<br>云端数据库"]
    end
...
```mermaid
flowchart TD
    subgraph AudioSources["音频源选择器"]
        TTS["TTS 引擎<br>AVSpeechSynth<br>即时合成"]
        HumanVoice["人声有声书<br>LibriVox<br>预录制音频"]
        AIVoice["AI 克隆语音<br>ElevenLabs<br>定制音色"]
    end

    TTS --> Controller
    HumanVoice --> Controller
    AIVoice --> Controller

    Controller["统一音频播放控制器<br>播放/暂停/停止 / 语速调节<br>章节跳转 / 循环播放"]
    Controller --> Highlight["高亮同步引擎<br>音频时间戳 -> 文本位置定位 -> UI高亮渲染"]
    Highlight --> AutoPage["自动翻页控制器<br>监测朗读完成 -> 触发翻页动画 -> 继续朗读"]
```
...
stateDiagram-v2
    [*] --> Reading: 朗读进行中
    Reading --> CheckPage: 监测当前页文本朗读完成
    CheckPage --> HasNext: 有下一页
    CheckPage --> ChapterEnd: 章节结束

    HasNext --> Pause100ms: 暂停朗读 (100ms)
    Pause100ms --> PageAnimation: 执行翻页动画
    PageAnimation --> ContinueRead: 继续朗读新页内容
    ContinueRead --> Reading

    ChapterEnd --> CheckNextChapter: 检测下一章设置
    CheckNextChapter --> AutoPlayNext: 自动播放下一章
    CheckNextChapter --> WaitUser: 暂停等待用户确认
    AutoPlayNext --> Reading
    WaitUser --> [*]
```

...
dependencies: [
    ...
    .package(url: "https://github.com/nicklockwood/SwiftFormat", from: "0.52.0"), // 代码格式化
]
```
...
class ReadingTimeTracker {
    ...
    private let idleThreshold: TimeInterval = 30 // 30秒无操作视为暂停
...
class AudiobookEngine {
    ...
    /// 跳转到指定位置
    func seekTo(textPosition: String) // 根据文字位置跳转音频
}
...
enum BadgeType: String, Codable {
    // 阅读时长勋章
    case reader_bronze      // 累计阅读1小时
    case reader_silver      // 累计阅读10小时
    case reader_gold        // 累计阅读50小时
    case reader_platinum    // 累计阅读100小时
    case reader_diamond     // 累计阅读500小时

    // 连续阅读勋章
    case streak_3days       // 连续阅读3天
    case streak_7days       // 连续阅读7天
    case streak_30days      // 连续阅读30天
    case streak_100days     // 连续阅读100天
    case streak_365days     // 连续阅读365天

    // 完成书籍勋章
    case bookworm_1         // 完成1本书
    case bookworm_5         // 完成5本书
    case bookworm_20        // 完成20本书
    case bookworm_50        // 完成50本书

    // 词汇勋章
    case vocab_100          // 掌握100个单词
    case vocab_500          // 掌握500个单词
    case vocab_1000         // 掌握1000个单词
    case vocab_5000         // 掌握5000个单词
    ...
    case speed_reader       // 阅读速度达到200词/分钟
    case thorough_reader    // 完成率100%
}
...
class AIRecommendViewModel: ObservableObject {
    ...
    @Published var aiBookLists: [BookList] = []  // AI生成的榜单
...
struct AILearningPlan: Codable {
    let dailyReadingGoal: Int      // 每日阅读目标(分钟)
    let weeklyBookGoal: Int        // 每周书籍目标
    let vocabularyGoal: Int        // 词汇目标
    let suggestedBooks: [Book]     // AI推荐的学习书单
...
struct BookList: Codable, Identifiable {
    ...
    let aiGenerationReason: String? // AI生成原因
    let books: [Book]?            // 榜单内书籍
    let createdAt: Date
    let updatedAt: Date
}
...
struct BookListItem: Codable {
    ...
    let position: Int
    let addedAt: Date
    let aiScore: Float?           // AI推荐分数
    let aiReason: String?         // AI推荐理由
}
```
...
struct BookListDetailView: View {
    ...
    var body: some View {
        ...
        .navigationTitle(viewModel.bookList?.name ?? "榜单详情")
        .task {
...
enum ReadingEventType {
    ...
    case sessionEnd             // 结束阅读
    case pageView               // 翻页
    case scrollPosition         // 滚动位置
    case pauseReading           // 暂停(切出App)
    case resumeReading          // 恢复阅读
    case chapterStart           // 开始新章节
    case chapterComplete        // 完成章节
    case bookComplete           // 完成全书
}
...
struct ReadingSessionData {
    ...
    let startTime: Date
    let endTime: Date?
    let durationSeconds: Int        // 实际阅读时长(秒)
    let activeDurationSeconds: Int  // 活跃阅读时长(去除暂停)
    ...
    let pauseCount: Int             // 暂停次数
    let pauseTotalSeconds: Int      // 总暂停时长
}
```

#### 9.2.2 AI交互数据 (iOS采集 → BE处理)
```swift
struct AIInteractionEvent {
    ...
    let responseTime: Int           // 响应时间(ms)
    let userAction: UserAction      // 用户后续操作
}

enum AIInteractionType {
    case wordExplain            // 查词
    case sentenceSimplify       // 简化句子
    case paragraphTranslate     // 段落翻译
    case contentQA              // 内容问答
}
...
enum UserAction {
    ...
    case copyText               // 复制内容
    case askFollowUp            // 追问
    case markUnderstood         // 标记已理解
    case markConfused           // 标记仍困惑
}
```

#### 9.2.3 理解程度数据 (iOS采集 → BE处理)
```swift
struct ComprehensionData {
    let userId: UUID
    let bookId: UUID
    let sentenceId: String          // 句子唯一标识
    let sentence: String
    let lookupCount: Int            // 查询次数
    let simplifyCount: Int          // 简化次数
    let translateCount: Int         // 翻译次数
    let timeSpentSeconds: Int       // 在该句停留时间
    let comprehensionLevel: Int     // 推断的理解程度 0-100
}
```
...
class LogManager {
    ...
    private let uploadThreshold = 100 // 满100条上传

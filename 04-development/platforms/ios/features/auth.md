...
```mermaid
flowchart TD
    A["Apple 登录"] --> B["ASAuthorizationController 授权"]
    B --> C["获取 Apple ID Token"]
    C --> D["发送 Token 到服务器"]
    D --> E["服务器验证并返回<br>Access Token + Refresh Token"]
    E --> F["存储到 Keychain"]
    F --> G["进入主页面"]
```
...
```mermaid
flowchart TD
    A["API 请求"] --> B["检查 Access Token"]
    B -->|"有效"| C["发送请求"]
    B -->|"过期/即将过期"| D["使用 Refresh Token 刷新"]
    D -->|"成功"| E["更新 Access Token"]
    E --> F["重试原请求"]
    D -->|"失败 (Refresh Token 过期)"| G["清除 Token"]
    G --> H["跳转登录页"]
```

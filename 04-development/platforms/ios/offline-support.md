...
├─────────────────────────────────────────────────┤
│  2. 图片缓存层 (Kingfisher)                     │
│  - 内存: 50MB  磁盘: 200MB  过期: 7天           │
│  - 书籍封面、作者头像                            │
├─────────────────────────────────────────────────┤
│  3. 数据缓存层 (ResponseCacheService)           │
│  - 内存缓存 + TTL 机制                          │
│  - API 响应数据                                  │
├─────────────────────────────────────────────────┤
│  4. 离线内容层 (ContentCache)                   │
│  - 已下载书籍的章节、封面、元数据                 │
│  - 文件系统持久化存储                            │
└─────────────────────────────────────────────────┘
```
...
1. 发起网络请求
   │
   ├── 成功 ──→ 更新UI + 缓存数据 + dataSource = .network
   │
   └── 失败 ──→ 检查缓存
                 │
                 ├── 有缓存 ──→ 加载缓存 + dataSource = .cache + 显示离线提示
                 │
                 └── 无缓存 ──→ 显示错误
```
...
页面加载
   │
   ├── dataSource == .network ──→ 正常显示
   │
   └── dataSource == .cache ──→ 显示 OfflineBannerView
                                 │
                                 ├── 显示上次同步时间
                                 │
                                 └── 提供刷新按钮 ──→ 重新请求网络

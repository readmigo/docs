...
```mermaid
flowchart LR
    A["用户交互<br>点击书签按钮<br>选中+添加<br>书签列表操作"] --> B["书签管理器<br>创建书签<br>位置计算<br>列表管理"]
    B --> C["存储/渲染<br>持久化存储<br>图标渲染<br>云端同步"]
```
...
```mermaid
flowchart TD
    A["用户点击书签按钮"] --> B["获取当前阅读位置<br>(CFI + 页码)"]
    B --> C{"检查是否已有书签"}
    C -->|"已有"| D["提示删除或跳转"]
    C -->|"没有"| E["显示添加书签面板<br>标题/颜色选择/保存"]
```

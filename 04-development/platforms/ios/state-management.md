...
graph TD
    subgraph ViewLayer["View Layer"]
        State["@State<br>局部状态"]
        Binding["@Binding<br>双向绑定"]
        Environment["@Environment<br>环境值"]
    end

    subgraph ViewModelLayer["ViewModel Layer"]
        Observable["@Observable<br>iOS 17+"]
        Published["@Published<br>可观察属性"]
        CombineP["Combine<br>Publisher"]
    end

    subgraph DataLayer["Data Layer"]
        SwiftData["SwiftData<br>持久化"]
        UserDef["UserDefaults<br>偏好设置"]
        Keychain["Keychain<br>安全存储"]
    end

    State --> Observable
    Binding --> Published
    Environment --> CombineP
...
flowchart TD
    Q1{"状态需要持久化?"}
    Q1 -->|"是"| A1["@AppStorage / SwiftData"]
    Q1 -->|"否"| Q2{"状态需要跨视图共享?"}
    Q2 -->|"是"| A2["@EnvironmentObject"]
    Q2 -->|"否"| Q3{"状态是复杂对象?"}
    Q3 -->|"是"| A3["@StateObject / @Observable"]
    Q3 -->|"否"| A4["@State"]
```
...
```mermaid
flowchart TD
    Request["网络请求"] --> Publisher["URLSession.dataTaskPublisher"]
    Publisher --> Decode["decode<br>JSON 解析"]
    Decode --> Map["map<br>数据转换"]
    Map --> Receive["receive(on: Main)<br>切换到主线程"]
    Receive --> Assign["assign<br>赋值到 @Published"]
```

...
flowchart TD
    ...
    B --> C["useQuery()<br>(Server State)"]
    B --> D["useStore()<br>(Client State)"]
    C --> E["React Query<br>Query Cache<br>Mutations"]
    D --> F["Zustand Store<br>State / Actions<br>Middleware (persist)"]
    E --> G["API Client<br>(Axios Instance)"]
    F --> H["AsyncStorage"]
    G --> I["Backend API<br>api.readmigo.app"]
```

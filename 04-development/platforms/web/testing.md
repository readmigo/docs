...
```mermaid
flowchart TD
    A["PR 提交"] --> B["Lint<br>ESLint + TypeScript 检查"]
    B --> C["Unit Tests<br>Vitest (并行)"]
    C --> D["Integration Tests<br>React Testing Library"]
    D --> E["Coverage<br>覆盖率报告上传"]
    E --> F["E2E (可选)<br>仅 main 分支或手动触发"]
```

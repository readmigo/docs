...
注意: 支付分析通过 Apple IAP / Google Play / Stripe 直接获取，未使用 RevenueCat。
...
| subscription_tier | String | 订阅等级 (FREE / PRO / PREMIUM) |
...
| book_id | String? | 来源书籍 |
| review_quality | Number | 复习质量 (0-5) |
...
| price | Number | 价格 (分) |
| currency | String | 货币 |
| plan_type | String | 套餐类型 (FREE / PRO / PREMIUM) |
...
| Premium Users | 订阅用户 (PRO / PREMIUM) |
| Free Users | 非订阅用户 |
...
```mermaid
graph TD
    A["app_launched (100%)"] --> B["onboarding_started (70%)"]
    B --> C["onboarding_completed (50%)"]
    C --> D["user_signed_up (30%)"]
    D --> E["first_book_read (20%)"]
```
...
```mermaid
graph TD
    A["paywall_viewed (100%)"] --> B["trial_started (25%)"]
    B --> C["trial_converted (10%)"]
```

...
```mermaid
graph TD
    A["用户请求<br>iOS/Android (90K) + Web App (10K)"] --> B["Cloudflare (CDN + WAF)<br>全球300+ PoP节点<br>静态资源缓存<br>DDoS防护 + 速率限制<br>Argo智能路由 (可选)"]
    B --> C["Fly.io NRT (Tokyo)<br>主区域 2实例"]
    B --> D["Fly.io HKG (HongKong)<br>中国优化 1实例"]
    B --> E["Fly.io SJC (美西)<br>美国优化 1实例"]
    C --> F["数据层<br>PostgreSQL (主库+只读副本)<br>Redis Upstash (集群模式/全球边缘)<br>Cloudflare R2 (封面+章节HTML+EPUB)"]
    D --> F
    E --> F
```
...
```mermaid
graph TD
    A["API Server (Fly.io)<br>HTTP请求处理<br>用户认证<br>数据查询"] --> C["Redis Queue (BullMQ)"]
    B["Worker Server (Fly.io)<br>AI数据生成<br>书籍导入<br>统计计算"] --> C
```
...
```mermaid
graph TD
    A["写请求<br>(INSERT/UPDATE/DELETE)"] --> B["主库 (NRT)<br>dedicated 1CPU / 2GB"]
    B -->|"异步复制"| C["只读副本<br>副本1 (NRT) shared 1x<br>副本2 (NRT) shared 1x"]
    D["读请求<br>(SELECT)"] --> C
```
...
```mermaid
graph TD
    A["API实例 (4个)<br>每实例连接数: 10<br>总连接需求: 40"] --> B["PgBouncer<br>连接复用<br>连接池: 50<br>模式: transaction"]
...
```mermaid
graph TD
    A["L1: 应用内存缓存 (Node.js)<br>用途: 极热点数据<br>容量: 50-100MB / TTL: 1-5分钟<br>数据: 分类树、热门书籍、配置项<br>实现: node-cache / lru-cache"] -->|"未命中"| B["L2: Redis分布式缓存<br>用途: 共享热点数据<br>容量: 500MB-1GB / TTL: 5分钟-24小时<br>数据: 书籍详情、作者信息、推荐列表、AI响应<br>实现: Upstash Redis (全球边缘)"]
    B -->|"未命中"| C["L3: CDN边缘缓存<br>用途: 静态资源<br>容量: 无限 / TTL: 1天-1年<br>数据: 封面图片、章节HTML、EPUB文件、字体<br>实现: Cloudflare R2 + CDN"]
```
...
```mermaid
graph TD
    A["用户请求图片"] --> B["检测设备/网络<br>屏幕DPR / 网络类型 / 视口大小"]

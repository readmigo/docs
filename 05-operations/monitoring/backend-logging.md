...
graph TD
    ...
    Q2 -->|"是"| INFO["INFO<br>(登录成功、支付完成等)"]
    Q2 -->|"否"| DEBUG1["DEBUG<br>(缓存命中、中间步骤等)"]
...
```mermaid
graph TD
    DEV["Development"] --> DEV_LVL["DEBUG 全量"]
    STG["Staging"] --> STG_LVL["DEBUG 全量"]
    PROD["Production"] --> PROD_LVL["INFO 默认"]
    PROD_LVL --> SAMPLE["DEBUG 采样<br>(1-10%)"]
    PROD_LVL --> TEMP["DEBUG 临时<br>(限时1小时)"]
```
...
```mermaid
graph TD
    BACKEND["Backend"] --> DB["Database<br>RuntimeLog"]
    DB --> EXPORTER["Exporter<br>(Cron)"]
    EXPORTER --> GRAFANA["Grafana Loki"]
    BACKEND -->|"ERROR/FATAL"| SENTRY["Sentry"]
    GRAFANA --> DASHBOARD["Grafana Dashboard<br>日志量趋势 / 错误分布<br>模块热力图 / 用户追踪"]
```
...
graph LR
    ...
    QUEUE --> BATCH["批量写入"]
    BATCH --> DB["Database"]
    QUEUE -.->|"队列满时降级<br>(丢弃 DEBUG)"| QUEUE
    BATCH -.->|"失败时重试<br>(最多 3 次)"| BATCH
```
...
graph TD
    ...
    JAEGER --> GRAFANA["Grafana<br>(Unified UI)"]
    PROMETHEUS --> GRAFANA
    LOKI --> GRAFANA
```

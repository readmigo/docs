...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ✅ 有声书导入的便捷入口（聚合 LibriVox）                     │
│  ✅ Top 100 榜单可作为初始内容筛选依据                        │
│  ✅ 12,000+ 英语有声书，超过产品 Phase 3 目标                │
│  ✅ 分类体系完善，便于内容组织                                │
│  ✅ 已有下载脚本，可直接使用                                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
│  ─────────────────────────────────────────────────────      │
│                                                             │
│  ① Top 100 热门榜      1 个          100 本精选              │
│                                                             │
│  ② 题材分类榜          13+ 个        每分类 Top 50-100       │
│     Fiction           → 100 本                              │
│     Literature        → 100 本                              │
│     Adventure         → 50 本                               │
│     Mystery           → 50 本                               │
...
```
┌─────────────────────────────────────────────────────────────┐
│               LoyalBooks vs 产品需求匹配                     │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Phase 1 需求: 700+ 精品书                                   │
│  ├── LoyalBooks Top 100 + 经典文学 ✅ 完全满足               │
│  └── 与 Standard Ebooks 高度重叠                            │
│                                                             │
│  Phase 2/3 需求: 5000+ 书籍                                  │
│  ├── LoyalBooks 英语有声书 12,129 本 ✅ 超额满足             │
│  └── 可筛选高质量子集                                        │
│                                                             │
│  有声书需求:                                                 │
│  ├── LoyalBooks 有完整音频（来自 LibriVox）                  │
│  └── 已有现成下载脚本 ✅                                     │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│  • 商业使用：明确允许，无需许可                               │
│  • 归属要求：希望但不强制                                     │
│  • ⚠️ 限制：仅保证美国公版，其他国家需自行确认               │
│                                                             │
│  Project Gutenberg 电子书 ⚠️ 中等风险                       │
│  ─────────────────────────────────────────────              │
│  • 文本内容：美国公版                                        │
│  • 商业使用：需移除 "Project Gutenberg" 商标                 │
│  • 保留商标：需支付版税                                      │
│  • ⚠️ 限制：部分书籍仍有版权，需逐本确认                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
│                                                             │
│  ☑️ 内容处理:                                                │
│     □ 移除所有 "Project Gutenberg" 标识                      │
│     □ 使用 LibriVox 原始音频（无需修改）                     │
│     □ 封面使用原始/自制，避免现代设计                        │
│                                                             │
│  ☑️ 文档记录:                                                │
│     □ 记录每本书的版权状态证明                               │
...
```
scripts/loyalbooks-ingestion/
├── download-all.ts      # 全量下载脚本
├── package.json         # 依赖配置
└── run-on-droplet.sh    # 服务器执行脚本
```
...
│                                                             │
│  Step 1: 获取书籍列表                                        │
│  └── 解析 sitemap.xml → ~35,783 书籍 URL                    │
│                                                             │
│  Step 2: 筛选有声书                                          │
│  └── 检查是否有 archive.org 音频 → ~12,000 有声书            │
│                                                             │
│  Step 3: 提取元数据                                          │
│  └── 解析书籍页面 → 标题/作者/描述/分类                      │
│                                                             │
│  Step 4: 下载音频                                            │
│  └── 从 archive.org 下载 MP3 章节                           │
│                                                             │
│  Step 5: 上传 R2                                             │
│  └── 上传音频/封面/元数据到 Cloudflare R2                   │
│                                                             │
│  Step 6: 同步数据库                                          │
│  └── 写入 audiobooks + audiobook_chapters 表                │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│  ├── 导入 Top 100 榜单                                       │
│  ├── 导入 Fiction Top 100                                    │
│  └── 导入 Literature Top 100                                 │
│      → 约 300 本高质量有声书                                 │
│                                                             │
│  中期（Phase 2）:                                            │
│  ├── 导入 Adventure/Mystery/Children 各 Top 50              │
│  ├── 导入 History/Biography 各 Top 50                        │
│  └── 补充经典作家全集                                        │
│      → 累计约 800 本                                         │
│                                                             │
│  长期（Phase 3）:                                            │

...
│  1. 从 R2 下载文件                                                    │
│  2. 检测文件格式 → MOBI/AZW3                                          │
│  3. 调用 MobiParser.parse()                                          │
│     ├─ 检测 Calibre 可用性                                            │
│     ├─ [Calibre 可用] ebook-convert → EPUB                           │
│     │   └─ EpubParser.parse() → ParsedBook                           │
│     └─ [Calibre 不可用] 回退解析 → 基础 ParsedBook                     │
│  4. 创建 Book 记录                                                    │
│  5. 创建 Chapter 记录                                                 │
│  6. 上传封面和章节内容到 R2                                            │
│  7. 更新用户配额                                                       │
│  8. 更新 ImportJob 状态为 COMPLETED                                   │
│  9. 清理临时文件                                                       │
└──────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼

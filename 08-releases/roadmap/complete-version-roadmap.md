...
│      ▼             ▼             ▼             ▼             ▼                        │
│  ┌───────┐    ┌───────┐    ┌───────┐    ┌───────┐    ┌───────┐                      │
│  │  MVP  │───►│ 有声书 │───►│智能阅读│───►│内容导入│───►│社区创作│                      │
│  └───────┘    └───────┘    └───────┘    └───────┘    └───────┘                      │
│      │             │             │             │             │                        │
│      ▼             ▼             ▼             ▼             ▼                        │
│   ┌─────┐      ┌─────┐      ┌─────┐      ┌─────┐      ┌─────┐                        │
│   │ iOS │      │+Andr│      │+Web │      │全平台│      │全平台│                        │
│   └─────┘      └─────┘      └─────┘      └─────┘      └─────┘                        │
│                                                                                       │
│   300书         +150音频      +增强数据     +用户内容     +社区内容                      │
│   100作者       +15GB        +1GB         用户驱动      用户驱动                        │
│                                                                                       │
└─────────────────────────────────────────────────────────────────────────────────────┘
```
...
│   │   ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌─────────┐     │ │
│   │   │  书库   │  │  作者   │  │  阅读   │  │  我的   │     │ │
│   │   │ Library │  │ Authors │  │ Reader  │  │ Profile │     │ │
│   │   └─────────┘  └─────────┘  └─────────┘  └─────────┘     │ │
│   │        │            │            │            │           │ │
│   │        ▼            ▼            ▼            ▼           │ │
│   │   • 300本书    • 100作者    • EPUB渲染   • 登录注册      │ │
│   │   • 分类浏览   • 作者详情   • 翻页阅读   • 阅读统计      │ │
│   │   • 搜索      • 作品列表   • 高亮标注   • 生词本        │ │
│   │   • 推荐      • 作者关注   • AI查词     • 进度同步      │ │
│   │   • 收藏                   • 书签                        │ │
...
│        ▼                  ▼                  ▼                   │
│   ┌──────────┐      ┌──────────┐      ┌──────────┐              │
│   │ iOS 1.x  │      │ iOS 2.x  │      │Android 2.x│             │
│   └────┬─────┘      └────┬─────┘      └────┬─────┘              │
│        │                  │                  │                   │
│        ▼                  ▼                  ▼                   │
│   v1.0.0.json        v2.0.0.json        v2.0.0.json             │
│        │                  │                  │                   │
│        ▼                  ▼                  ▼                   │
│   无有声书 Tab        有有声书 Tab       有有声书 Tab            │
│   访问: epubs/        访问: epubs/       访问: epubs/            │
...
```
┌─────────────────────────────────────────────────────────────────┐
│              原则: EPUB 文件保持不变，增强数据分离               │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│   ❌ 不修改 EPUB                  ✅ Overlay 叠加层              │
│   ├── 破坏原文件完整性           ├── 原文件不变                 │
│   ├── 无法回滚                   ├── 增强数据独立存储           │
│   ├── 版本管理复杂               ├── 客户端运行时叠加           │
│   └── 多客户端同步困难           └── 易于升级和回滚             │
...
│    → timestamps.json, hotwords.json, paragraphs.json│
└─────────────────────────────────────────────────────┘
      │
      ▼
┌─────────────────────────────────────────────────────┐
...
│  │有声书 │   │Android│   │播放增强│   │离线下载│   │关联优化│                              │
│  └───────┘   └───────┘   └───────┘   └───────┘   └───────┘                              │
│      │           │           │           │           │                                    │
│      ▼           ▼           ▼           ▼           ▼                                    │
│    iOS       iOS+Andr    iOS+Andr    iOS+Andr    iOS+Andr                                │
│                                                                                           │
│  V3.x Smart                                                                               │
│  ────────────────────────────────────────────────────────────────────────────────────    │
│  │ 3.0.0 │──►│ 3.1.0 │──►│ 3.2.0 │──►│ 3.3.0 │──►│ 3.4.0 │                              │
│  │系统TTS│   │热词标注│   │段落解析│   │边读边听│   │Web App│                              │
│  └───────┘   └───────┘   └───────┘   └───────┘   └───────┘                              │
│      │           │           │           │           │                                    │
│      ▼           ▼           ▼           ▼           ▼                                    │
│  iOS+Andr    iOS+Andr    iOS+Andr    iOS+Andr    全平台                                  │
│                                                                                           │
│  V4.x Import                                                                              │
│  ────────────────────────────────────────────────────────────────────────────────────    │
│  │ 4.0.0 │──►│ 4.1.0 │──►│ 4.2.0 │──►│ 4.3.0 │──►│ 4.4.0 │                              │
│  │EPUB导入│   │PDF导入│   │网页保存│   │导入增强│   │批量同步│                              │
│  └───────┘   └───────┘   └───────┘   └───────┘   └───────┘                              │
│      │           │           │           │           │                                    │
│      ▼           ▼           ▼           ▼           ▼                                    │
│   全平台       全平台       全平台       全平台       全平台                                │

...
```mermaid
graph LR
    subgraph Collect["内容采集层"]
        C1["RSS抓取<br>API调用<br>用户投稿"]
    end

    subgraph Process["内容处理层"]
        P1["版权分级<br>AI摘要生成<br>难度评估"]
    end

    subgraph Display["内容展示层"]
        D1["分类浏览<br>订阅Feed<br>阅读器"]
    end

    subgraph Learn["学习功能层"]
        L1["生词标注<br>阅读历史"]
    end

    subgraph Community["社区功能层"]
        CM1["关注/订阅<br>分享"]
    end

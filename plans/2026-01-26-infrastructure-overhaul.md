...
```mermaid
graph TB
    subgraph Envs["当前环境架构 (4 环境)"]
        L["Local<br>localhost"] --> LDB["本地 PG<br>本地 Redis<br>R2 debug"]
        D["Debugging<br>Fly.io nrt"] --> DDB["Neon Debug<br>Upstash<br>R2 debug"]
        S["Staging<br>Fly.io nrt"] --> SDB["Neon Staging<br>Upstash<br>R2 staging"]
        P["Production<br>Fly.io nrt"] --> PDB["Neon Prod<br>Upstash<br>R2 prod"]
    end
```
...
```mermaid
graph TB
    subgraph Target["目标环境架构 (2 环境)"]
        L2["Local<br>localhost:3000"] --> LDB2["本地 PostgreSQL<br>本地 Redis<br>R2 readmigo-dev"]
        P2["Production<br>api.readmigo.app"] --> PDB2["Neon PostgreSQL<br>Upstash Redis<br>R2 readmigo-prod"]
    end
```
...
```mermaid
graph TB
    CF["Cloudflare<br>(CDN/DNS)"] --> Fly["Fly.io API<br>Tokyo (nrt)<br>512MB x 1+"]
    Fly --> Neon["Neon PostgreSQL<br>Singapore"]
    Fly --> Upstash["Upstash Redis<br>Global"]
    Fly --> R2["Cloudflare R2<br>Global"]
```
...
```mermaid
graph TB
    P1["阶段 1: 当前 (MVP)<br>Fly.io Tokyo 单点<br>重点服务亚洲用户<br>成本最低"]
    P2["阶段 2: 用户增长后<br>添加 Fly.io US-West 节点<br>数据库读副本<br>静态资源 CDN 优化"]
    P3["阶段 3: 规模化<br>Cloudflare Workers 边缘 API<br>数据库多区域同步<br>完整的全球化架构"]

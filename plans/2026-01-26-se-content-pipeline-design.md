...
| 能力 | Standard Ebooks | Readmigo |
|------|----------------|----------|
| 高质量内容 | Yes | Yes (来自 SE) |
| 排版规范 | Yes | Yes (继承 SE) |
| 原生客户端 | No | Yes (iOS/Android) |
| 阅读体验优化 | No | Yes (翻页动画、主题、字体) |
| AI 功能 | No | Yes (双击翻译、AI 解释) |
| 学习功能 | No | Yes (生词本、进度同步) |
| TTS | No | Yes (TTS 朗读、音文同步) |
...
```mermaid
graph TB
    subgraph Content["内容层 (Standard Ebooks)"]
        SE["1000+ 高质量公共领域书籍<br>完美排版 / 干净 HTML 结构"]
    end

    subgraph Service["服务层 (Readmigo Backend)"]
        Imp["Importer<br>SE同步 / 元数据提取"] --> Adp["Adapter<br>段落ID注入 / 图片路径适配"]
        Adp --> Pub["Publisher<br>数据库+R2存储"]
    end

    subgraph App["体验层 (Readmigo App)"]
        Client["iOS / Android 原生客户端<br>书库浏览 / 阅读器 / AI 功能<br>学习功能 / TTS / 社区功能"]
    end
